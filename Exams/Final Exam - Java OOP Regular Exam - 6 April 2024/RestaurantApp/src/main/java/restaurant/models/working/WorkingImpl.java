    ...
    public void takingOrders(Client client, Collection<Waiter> waiters) {
        for (Waiter waiter : waiters) {
            while (waiter.canWork() && !client.getClientOrders().isEmpty()) {
                ...
                waiter.takenOrders().getOrdersList().add(order); // сервитъора взима поръчката
                client.getClientOrders().remove(order);  // и я изтриваме от исканите поръчки на клиента, защото вече
                                                         //                                            сме я взели
            }
        }
    }
